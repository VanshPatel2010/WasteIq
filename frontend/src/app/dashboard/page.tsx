...
export default function DashboardPage() {
  ...
  const metricCards = stats ? [
    { label: "Total Zones", value: stats.total_zones, sub: `${stats.overdue_zones} overdue`, color: "text-[#1B7A4A]", bg: "bg-[#1B7A4A]/10" },
    { label: "Active Trucks", value: `${stats.active_trucks}/${stats.total_trucks}`, sub: "on route", color: "text-[#15803D]", bg: "bg-[#15803D]/10" },
    { label: "Surge Alerts", value: stats.surge_alerts, sub: stats.surge_alerts > 0 ? "requires attention" : "all clear", color: stats.surge_alerts > 0 ? "text-[#B91C1C]" : "text-[#15803D]", bg: stats.surge_alerts > 0 ? "bg-[#B91C1C]/10" : "bg-[#15803D]/10" },
    { label: "Surplus Matches", value: stats.surplus_matches_today, sub: "today", color: "text-[#92702A]", bg: "bg-[#92702A]/10" },
    { label: "Worker Reports", value: stats.worker_reports_today, sub: `${stats.zones_covered_today} zones covered`, color: "text-[#15803D]", bg: "bg-[#15803D]/10" },
  ] : [];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-xs text-[#6B7280] bg-[#F0EDE6] px-3 py-2 rounded-lg font-medium border border-[#D6D3C8]">
      ...
      <div className="grid grid-cols-5 gap-6">
        ...
        <div className="col-span-2 space-y-4">
          ...
          <div className="card">
            <h3 className="text-sm font-semibold mb-3 text-[#1F2937]">🚛 Route Progress</h3>
            {trucks.map((t: any) => (
              <div key={t.id} className="py-2 border-b border-[#D6D3C8] last:border-0 hover:bg-[#F0EDE6]/30 px-2 -mx-2 rounded-lg transition-colors">
                <div className="flex justify-between text-sm">
                  <span className="text-[#1F2937] font-medium">{t.vehicle_number}</span>
                  <span className={`badge ${t.status === "on_route" ? "badge-worker" : t.status === "completed" ? "fill-green" : "badge-driver"}`}>{t.status}</span>
                </div>
                {t.driver_name && <p className="text-xs text-[#6B7280] mt-1">{t.driver_name}</p>}
              </div>
            ))}
          </div>
        </div>
